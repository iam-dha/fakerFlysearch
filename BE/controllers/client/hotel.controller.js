    ...
    try {
        ...
        for (const r of rooms) {
            ...
            if (!room || room.deleted)
                return res
                    .status(404)
                    .json({ message: `Room not found: ${r.room}` });

            if (room.available_rooms < r.quantity) {
                return res.status(400).json({
                    message: `Not enough rooms for type: ${room.type}`,
                });
            }
        ...
        if (user?.email) {
            await mailer.sendHotelBookingConfirmation(
                user.email,
                booking,
                hotel
            );
        ...
        return res
            .status(201)
            .json({ message: "Booking successful", data: booking });
    } catch (err) {
...
exports.getHotelsByIata = async (req, res) => {
    ...
    try {
        ...
        if (hotels.length === 0) {
            return res
                .status(404)
                .json({ message: "No hotels found for this IATA code" });
        }
...
exports.getHotelById = async (req, res) => {
    ...
    try {
        ...
        hotel.rooms = await Room.find({ hotel: hotelId, deleted: false })
            .select("-__v -hotel")
            .lean();
        return res.status(200).json({
        ...
        console.error(`[GET /hotels/${hotelId}] Error:`, err);
        return res.status(500).json({ message: "Internal server error" });
    }
};
